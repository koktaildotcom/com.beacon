...
class BeaconDriver extends Homey.Driver {
     ...
     * @abstract
     *
     * The type of beacon handled by this driver. The beacon types are coded in 
     * the module BeaconAdvertisement.js.
     */
    get handledBeaconType() {
        throw new Error('todo: Implement getHandledBeaconType into child class');
    }
...
//                const found = (advertisementUuids.indexOf(device.getData().uuid) !== -1);
    ...
    isDeviceInAdvertisements(device, beaconAdvs) {
        for (let beaconAdv of beaconAdvs) {
            if (this.getDevice(beaconAdv.key) == device) {
                device.updateCapabilityValues(beaconAdv);
                return true;
            }
        }
        return false;
