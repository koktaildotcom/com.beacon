    ...
    onRepair(socket, device) {
        socket.on('query_curr_device', (data, callback) => {
            let beaconTypeAndDetails = {
                ...
                typeName: device.getSetting('typeName'),
        ...
        socket.on('discover_and_refresh', (data, callback) => {
            ...
            Homey.app.discoverDevices(this)
                .then(devices => {
                    let devToBeRefreshed = null;
                    for (let i = 0; i < devices.length; i++) {
                        let devToBePaired = devices[i];
                        if (this.getDevice(devToBePaired.data) == device) {
                            devToBeRefreshed = devToBePaired;
                            break;
                        }
                    }
                    if (devToBeRefreshed) {
                        callback(null, device.refreshSettings(devToBeRefreshed));
                    }
                    else {
                        callback(null, null);
                    }
