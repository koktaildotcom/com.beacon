...
class BeaconDriver extends Homey.Driver {
    ...
    _updateDevice(device) {
        return new Promise((resolve, reject) => {
            try {
                this._getInRange(device)
                    .then(found => {
                        if(device.getCapabilityValue("detect") !== found){
                            if(found){
                                console.log('Device: ' + device.getName() + ' not found');
                                device.setDetect();
                            }
                            else{
                                console.log('Device: ' + device.getName() + ' found');
                                device.setUndetect();
                            }
                        }
    ...
    _getInRange(device) {
        return new Promise((resolve) => {
            Homey.app.discover(device)
                .then((device) => {
                    console.log('device discovered: %s', device.getName());
                    resolve(true);
                })
                .catch(error => {
                    console.log('device not discovered: %s', error);
                    resolve(false);
                });
        });
    }
