...
class BeaconDriver extends Homey.Driver {
    ...
    isMyAdvertisement(advertisement) {
        throw new Error('todo: Implement `isMyAdvertisement` into child class');
    }
    ...
    onInit() {
        ...
        this.homey.on('update.beacon.status', advertisements => {
            let myAdvertisements = advertisements.filter(this.isMyAdvertisement);
            for (const device of driver.getDevices()) {
                ...
                for (const myAdvertisement of myAdvertisements) {
                    if (device.matchAdvertisement(myAdvertisement)) {
                        const beacon = device.parseAdvertisement(myAdvertisement);
                ...
                if (detected) {
                    this.log(device.getName() + '[✓]');
                    device.setDetect();
                } else {
                    this.log(device.getName() + '[x]');
                    device.setUndetect();
                }
            }
        });

        this.homey.app.sendLog();
    }
    ...
    async onPair(session) {
        ...
        session.setHandler('list_devices', async () => {
            try {
                const advertisements = await this.homey.app._discoverAdvertisements();
                const myAdvertisements = advertisements.filter(this.isMyAdvertisement);
                const devices = [];
                for (const myAdvertisement of myAdvertisements) {
