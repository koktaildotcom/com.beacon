...
class BeaconDevice extends Homey.Device {
   ...
   /**
    ...
    parseAdvertisement(advertisement) {
        ...
        // The Feasycom devices (whose public MAC address starts with DC:0D) can
        //  send battery level percentage within a service. The service UUID is
        //  0xFFF0 which is proprietary (it does not appear to be a member
        //  service of the Bluetooth SIG).
        if (advertisement.address.startsWith('dc:0d')) {
            if (advertisement.serviceData !== undefined) {
                for (const serviceDatum of advertisement.serviceData) {
                    if (serviceDatum.uuid == 'fff0') {
                        if (serviceDatum.data.length >= 11) {
                            currBatteryLevel = serviceDatum.data[10];
                        }
                    }
                }
            }
