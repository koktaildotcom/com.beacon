    ...
    setDetect() {
        if (this.getCapabilityValue("detect") === false) {
            ...
            if (this.changeDetected >= Homey.ManagerSettings.get('verificationAmountInside')) {
                ...
                this.beaconStateChanged.trigger(this, {
                    'beacon': this.getName(),
                    'detected' : true
                })
                    .catch(function (error) {
                        Homey.app.log('Cannot trigger flow card beacon_state_changed: %s.', error);
                    });

                this.deviceBeaconStateChanged.trigger(this, {
                    'beacon': this.getName(),
                    'detected' : true
                })
                    .catch(function (error) {
                        Homey.app.log('Cannot trigger flow card device_beacon_state_changed: %s.', error);
                    });
            }
        }
        else {
